...
int lteCCA_sum_tbs(srslte_ue_cell_usage* q, int cell_idx, int len, int* ue_tbs1, int* ue_tbs2, int* ue_tbs_hm1, int* ue_tbs_hm2){
    uint16_t cell_header = q->header;
    int	     ue_tbs1_sum	= 0;
    int	     ue_tbs2_sum	= 0;
    int	     ue_tbs_hm1_sum	= 0;
    int	     ue_tbs_hm2_sum	= 0;
	...
	int idx = TTI_TO_IDX(index);
	ue_tbs1_sum	+= (int) (q->cell_status[cell_idx].sf_status[idx].tbs_tb1);

	ue_tbs2_sum	+= (int) (q->cell_status[cell_idx].sf_status[idx].tbs_tb2);

	ue_tbs_hm1_sum	+= (int) (q->cell_status[cell_idx].sf_status[idx].tbs_hm_tb1);

	ue_tbs_hm2_sum	+= (int) (q->cell_status[cell_idx].sf_status[idx].tbs_hm_tb2);
    }

    *ue_tbs1	= ue_tbs1_sum;
    *ue_tbs2	= ue_tbs2_sum;
    *ue_tbs_hm1	= ue_tbs_hm1_sum;
    *ue_tbs_hm2	= ue_tbs_hm2_sum;
    return 0;
}
int lteCCA_tuning_tbs(int tbs){
    ...
    int rateM       = tbs / 1000;
    if(rateM <=0){ rateM = 1;}
    if(rateM > 100){ rateM = 100;}

    float overhead  = overhead_ratio_phy[5][rateM-1] + protocol_overhead;
    //printf("overhead: %f \n", overhead);
...
int lteCCA_predict_tbs_single_cell(srslte_ue_cell_usage* q, int cell_idx, int* tbs, int* tbs_hm, int* full_load, int *full_load_hm){
    ...
    lteCCA_sum_tbs(q, cell_idx, AVE_UE_PHY_RATE, &ue_tbs1, &ue_tbs2, &ue_tbs_hm1, &ue_tbs_hm2);
    ue_sum_prb		= lteCCA_sum_ue_prb(q, cell_idx, AVE_UE_PHY_RATE);

    int ue_phy_rate1	= 0;
    int ue_phy_rate2	= 0;
    int ue_phy_rate_hm1	= 0;
    int ue_phy_rate_hm2	= 0;

    if(ue_sum_prb <= 0){
	// the UE is not active in the past AVE_UE_PHY_RATE subframes,
	// we thus use the rate from last round
	ue_phy_rate1	= q->cell_status[cell_idx].last_rate_tbs1;
	ue_phy_rate2	= q->cell_status[cell_idx].last_rate_tbs2;
	ue_phy_rate_hm1	= q->cell_status[cell_idx].last_rate_hm1;
	ue_phy_rate_hm2	= q->cell_status[cell_idx].last_rate_hm2;

    }else{
	// otherwise, we use the calculated rate and update the last rate
	ue_phy_rate1	= ue_tbs1 / ue_sum_prb;
	ue_phy_rate2	= ue_tbs2 / ue_sum_prb;
	ue_phy_rate_hm1	= ue_tbs_hm1 / ue_sum_prb;
	ue_phy_rate_hm2	= ue_tbs_hm2 / ue_sum_prb;

	q->cell_status[cell_idx].last_rate_tbs1	= ue_phy_rate1;
	q->cell_status[cell_idx].last_rate_tbs2	= ue_phy_rate2;
	q->cell_status[cell_idx].last_rate_hm1	= ue_phy_rate_hm1;
	q->cell_status[cell_idx].last_rate_hm2	= ue_phy_rate_hm2;
...
//	    cell_empty_prb, ave_ue_prb, ue_tbs, ue_tbs_hm, ue_phy_rate, ue_phy_rate_hm);
    ...
    int exp_tbs2	    = exp_available_prb * ue_phy_rate2;	// expected tbs for the ue
    ...
    int full_load_tbs2		= max_cell_prb  * ue_phy_rate2;
    int full_load_tbs_hm1	= max_cell_prb  * ue_phy_rate_hm1 ;
    int full_load_tbs_hm2	= max_cell_prb  * ue_phy_rate_hm2;

    int tuned_full_load_tbs	=  lteCCA_tuning_tbs(full_load_tbs1) + lteCCA_tuning_tbs(full_load_tbs2);
    int tuned_full_load_tbs_hm	=  lteCCA_tuning_tbs(full_load_tbs_hm1) + lteCCA_tuning_tbs(full_load_tbs_hm2);
...
int lteCCA_average_ue_rate(srslte_ue_cell_usage* q, int* rate, int* rate_hm){
    ...
    lteCCA_sum_tbs(q, 0, AVE_UE_RATE, &ue_tbs1, &ue_tbs2, &ue_tbs_hm1, &ue_tbs_hm2);
    ue_tbs1	= ue_tbs1 / AVE_UE_RATE;
	...
	for(int i=1;i<nof_cell;i++){
	    if(q->cell_triggered[i]){
		lteCCA_sum_tbs(q, i, AVE_UE_RATE, &ue_tbs1, &ue_tbs2, &ue_tbs_hm1, &ue_tbs_hm2);
		ue_tbs1	= ue_tbs1 / AVE_UE_RATE;
		ue_tbs2	= ue_tbs2 / AVE_UE_RATE;
		ue_tbs_hm1	= ue_tbs_hm1 / AVE_UE_RATE;
		ue_tbs_hm2	= ue_tbs_hm2 / AVE_UE_RATE;
