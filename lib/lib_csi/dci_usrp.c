...
extern bool    rf_ready_G[MAX_NOF_USRP];
...
void* dci_start_usrp(void* p)
{
    ...
    // set RF parameters for the shared parameters
    pthread_mutex_lock(&mutex_RF_config);
    cell_prb_G[usrp_idx]    = cell[usrp_idx].nof_prb;
    nof_tx_ant_G[usrp_idx]  = cell[usrp_idx].nof_ports;
    rf_ready_G[usrp_idx]    = true;
