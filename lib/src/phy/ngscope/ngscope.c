...
void unpack_dci_message_vec(srsran_ue_dl_t *q, srsran_dl_sf_cfg_t *sf,
                            srsran_ue_dl_cfg_t *cfg,
                            srsran_pdsch_cfg_t *pdsch_cfg,
                            srsran_dci_msg_t dci_msg[MAX_NOF_FORMAT],
                            int nof_dci, int loc_idx, ngscope_tree_t *tree) {
        ...
        for (int j = 0; j < nof_dci; j++) {
            ...
            if (dci_msg[j].format == SRSRAN_DCI_FORMAT0) {
                ...
                if (srsran_ngscope_unpack_ul_dci_2grant(
                        q, sf, cfg, pdsch_cfg, &dci_msg[j], &dci_ul,
                        &dci_ul_grant) == SRSRAN_SUCCESS) {
                    srsran_ngscope_dci_into_array_ul(
                        tree->dci_array, 0, loc_idx,
                        tree->dci_location[loc_idx], dci_msg[j].decode_prob,
                        dci_msg[j].corr, &dci_ul, &dci_ul_grant);
                }
            } else {
                // Upack the downlink dci to downlink grant
                if (srsran_ngscope_unpack_dl_dci_2grant(
                        q, sf, cfg, pdsch_cfg, &dci_msg[j], &dci_dl,
                        &dci_dl_grant) == SRSRAN_SUCCESS) {
                    int format_idx = ngscope_format_to_index(dci_msg[j].format);
                    srsran_ngscope_dci_into_array_dl(
                        tree->dci_array, format_idx, loc_idx,
                        tree->dci_location[loc_idx], dci_msg[j].decode_prob,
                        dci_msg[j].corr, &dci_dl, &dci_dl_grant);
                }
            }
        }
    }
...
void copy_dci_to_output(ngscope_tree_t *tree,
                        ngscope_dci_per_sub_t *dci_per_sub, int format_idx,
                        int loc_idx) {
    // copy the matched dci message to the results
    srsran_ngscope_tree_copy_dci_fromArray2PerSub(tree, dci_per_sub, format_idx,
                                                  loc_idx);
    ...
    // delete the messages in the dci array (including matched root and its
    // children)
    srsran_ngscope_tree_clear_dciArray_nodes(tree, loc_idx);

    return;
}
int child_parent_match(ngscope_tree_t *tree, ngscope_dci_per_sub_t *dci_per_sub,
    ...
    int matched_root = -1;
    int nof_matched = 0;
    srsran_ngscope_tree_CP_match(tree, blk_idx, loc_idx, targetRNTI,
                                 &nof_matched, &matched_root,
                                 matched_format_vec);
    // srsran_ngscope_tree_CP_match(tree.dci_array, nof_location, blk_idx,
    // loc_idx, &nof_matched, &matched_root, matched_format_vec);

    /* Pruning a single node. We can only decode one dci from each location, so
    we need to figure out which format is correct if there are mutliple dci are
    decoded from that specific location. */
    if (nof_matched > 0) {
        ...
        if (nof_matched > 1) {
            // Prune the nodes since it is possible that two RNTIs are matched
            // for one node
            pruned_nof_dci = srsran_ngscope_tree_prune_node(
                tree, nof_matched, matched_root, targetRNTI, matched_format_vec,
                &format_idx);
        }
        if (pruned_nof_dci == 1) {
            ...
            if ((format_idx != 0) && (format_idx != 4)) {
                ...
                // space_match =
                // sr
                // sran_ngscope_space_match_yx(dci_array[format_idx][matched_root].rnti,
                // \
				//                nof_cce, sf->tti % 10,
                //                 dci_location[matched_root].ncce,
                //                 ngscope_index_to_format(format_idx));
            }
            if (space_match) {
                found_dci++;
...
int srsran_ngscope_search_all_space_array_yx(
    srsran_ue_dl_t *q, srsran_dl_sf_cfg_t *sf, srsran_ue_dl_cfg_t *cfg,
    srsran_pdsch_cfg_t *pdsch_cfg,
    // ngscope_dci_msg_t      dci_array[][MAX_CANDIDATES_ALL],
    // srsran_dci_location_t  dci_location[MAX_CANDIDATES_ALL],
    ngscope_dci_per_sub_t *dci_per_sub, ngscope_tree_t *tree,
    uint16_t targetRNTI) {
    ...
    while (loc_idx < tree->nof_location) {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (tree->dci_location[loc_idx].checked ||
                tree->dci_location[loc_idx].mean_llr < LLR_RATIO) {
            ...
            if (search_space.loc[0].ncce == 0) {
                ...
                int nof_dci = srsran_ngscope_search_in_space_yx(
                    q, sf, &search_space, &dci_cfg, dci_msg);

                // Unpack the dci messages
                unpack_dci_message_vec(q, sf, cfg, pdsch_cfg, dci_msg, nof_dci,
                                       loc_idx, tree);

                int format_idx = srsran_ngscope_tree_find_rnti_range(
                    tree, loc_idx, 0xFFF4, 0xFFFF);
                if (format_idx >= 0) {
            ...
            found_dci += child_parent_match(tree, dci_per_sub, loc_idx, blk_idx,
                                            targetRNTI);
            // printf("prune done!\n");
...
/* Yaxiong's dci search function */
int srsran_ngscope_search_all_space_array_signleUE_yx(
    srsran_ue_dl_t *q, srsran_dl_sf_cfg_t *sf, srsran_ue_dl_cfg_t *cfg,
    srsran_pdsch_cfg_t *pdsch_cfg, ngscope_dci_per_sub_t *dci_per_sub,
    uint16_t targetRNTI) {
    int ret = SRSRAN_ERROR;
    // int nof_location = 0;
    int nof_ul_dci = 0;
            ...
            if (nof_dci > 0) {
                for (int j = 0; j < nof_dci; j++) {
                    if (dci_msg[j].format == SRSRAN_DCI_FORMAT0) {
                        // Upack the uplink dci to uplink grant
                        if (srsran_ngscope_unpack_ul_dci_2grant(
                                q, sf, cfg, pdsch_cfg, &dci_msg[j],
                                &dci_ul[nof_ul_dci],
                                &dci_ul_grant[nof_ul_dci]) == SRSRAN_SUCCESS) {
                            srsran_ngscope_dci_into_array_ul(
                                tree.dci_array, 0, loc_idx,
                                tree.dci_location[loc_idx],
                                dci_msg[j].decode_prob, dci_msg[j].corr,
                                &dci_ul[nof_ul_dci], &dci_ul_grant[nof_ul_dci]);
                        }
                    } else {
                        // Upack the downlink dci to downlink grant
                        if (srsran_ngscope_unpack_dl_dci_2grant(
                                q, sf, cfg, pdsch_cfg, &dci_msg[j],
                                &dci_dl[nof_dl_dci],
                                &dci_dl_grant[nof_dl_dci]) == SRSRAN_SUCCESS) {
                            int format_idx =
                                ngscope_format_to_index(dci_msg[j].format);
                            srsran_ngscope_dci_into_array_dl(
                                tree.dci_array, format_idx, loc_idx,
                                tree.dci_location[loc_idx],
                                dci_msg[j].decode_prob, dci_msg[j].corr,
                                &dci_dl[nof_dl_dci], &dci_dl_grant[nof_dl_dci]);
                        }
                    }
                }
            }
...
int srsran_ngscope_decode_SIB_yx(srsran_ue_dl_t *q, srsran_dl_sf_cfg_t *dl_sf,
    ...
    n = srsran_ue_dl_find_and_decode(q, dl_sf, ue_dl_cfg, pdsch_cfg, data,
                                     acks);
