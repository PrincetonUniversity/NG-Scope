                                            ...
                                            uint16_t               targetRNTI)
{
  ...
  if (nof_matched > 0) {
    ...
    if (nof_matched > 1) {
      ...
      // for one node
      pruned_nof_dci =
          srsran_ngscope_tree_prune_node(tree, nof_matched, matched_root, targetRNTI, matched_format_vec, &format_idx);
    }
    if (pruned_nof_dci == 1) {
...
bool check_node(srsran_dci_location_t dci_location[MAX_CANDIDATES_ALL], int start_idx, int idx_in_tree);
 ...
 *********************************************************/
bool clear_dciArray_child_node(ngscope_dci_msg_t dci_array[][MAX_CANDIDATES_ALL], int start_idx, int idx_in_tree);
...
bool clear_dciArray_parent_node(ngscope_dci_msg_t dci_array[][MAX_CANDIDATES_ALL], int start_idx, int idx_in_tree);
...
bool is_empty_node_regarding_llr(srsran_dci_location_t dci_location[MAX_CANDIDATES_ALL], int index)
{
                       ...
                       int                   index)
{
  ...
  if (is_empty_node_regarding_llr(dci_location, index) || is_empty_node(dci_array, index)) {
...
int srsran_ngscope_pick_single_location(ngscope_dci_msg_t dci_array[][MAX_CANDIDATES_ALL], int loc_idx)
{
  for (int i = 0; i < MAX_NOF_FORMAT + 1; i++) {
...
void srsran_ngscope_tree_plot_multi(ngscope_tree_t* q)
{
  ...
  while (loc_idx < q->nof_location) {
    for (int i = 0; i < 15; i++) {
      printf("|%d %d %.2f| ",
             q->dci_location[loc_idx].L,
             q->dci_location[loc_idx].ncce,
             q->dci_location[loc_idx].mean_llr);
      for (int j = 0; j < MAX_NOF_FORMAT + 1; j++) {
        printf("{%d %d %1.2f %1.2f}-",
               q->dci_array[j][loc_idx].rnti,
               q->dci_array[j][loc_idx].prb,
               q->dci_array[j][loc_idx].decode_prob,
               q->dci_array[j][loc_idx].corr);
      }
...
{
  for (int j = 0; j < MAX_NOF_FORMAT + 1; j++) {
    if ((q->dci_array[j][loc_idx].rnti >= rnti_min) && (q->dci_array[j][loc_idx].rnti <= rnti_max)) {
      return j;
    }
  }
  return -1;
                                   ...
                                   srsran_pdsch_grant_t* dci_dl_grant)
{
  srsran_ngscope_dci_into_array_dl(
      q->dci_array, format_idx, loc_idx, q->dci_location[loc_idx], decode_prob, corr, dci_dl, dci_dl_grant);
  return 0;
}
                                   ...
                                   srsran_pusch_grant_t* dci_ul_grant)
{
  srsran_ngscope_dci_into_array_ul(
      q->dci_array, format_idx, loc_idx, q->dci_location[loc_idx], decode_prob, corr, dci_ul, dci_ul_grant);
  return 0;
}
