...
void copy_dci_to_output(ngscope_tree_t* tree, ngscope_dci_per_sub_t* dci_per_sub, int format_idx, int loc_idx)
{
                       ...
                       uint16_t               targetRNTI)
{
  ...
  // srsran_ngscope_tree_CP_match(tree.dci_array, nof_location, blk_idx, loc_idx, &nof_matched, &matched_root,
  // matched_format_vec);
  ...
  if (nof_matched > 0) {
    ...
    if (pruned_nof_dci == 1) {
      ...
      if ((format_idx != 0) && (format_idx != 4)) {
        ...
        // space_match = sr sran_ngscope_space_match_yx(dci_array[format_idx][matched_root].rnti, \
                                                      ...
                                                      uint16_t               targetRNTI)
{
  ...
  ngscope_tree_set_locations(&tree, &q->pdcch, sf->cfi);
  // printf("NOF_LOC:%d cfi:%d nof_cce: %d %d %d\n", tree.nof_location, sf->cfi, q->pdcch.nof_cce[0],
  // q->pdcch.nof_cce[1], q->pdcch.nof_cce[2]); srsran_ngscope_tree_plot_loc(&tree);
