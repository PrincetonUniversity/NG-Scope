...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "tcpbRl")) != -1) {
        switch (opt) {
        case 't':
        ...
        case 'c':
            ...
            break;
	case 'p':
            pbe = atoi(argv[optind]);
            break;
	case 'b':
            bbr = atoi(argv[optind]);
            break;
        case 'l':
	...
	*************************************************************/
	...
	for(int i=0;i<6;i++){
	    ...
	    ret = system(cmd);
	    sprintf(cmd,"./mv_dci_trace.sh %d %d", trace_idx, i);
	    ret = system(cmd);
 
	    printf("TEST ONE CCA finished!\n");	
