...
pthread_mutex_t dci_plot_mutex[MAX_NOF_RF_DEV] = {PTHREAD_MUTEX_INITIALIZER,
                                                  PTHREAD_MUTEX_INITIALIZER,
                                                  PTHREAD_MUTEX_INITIALIZER,
                                                  PTHREAD_MUTEX_INITIALIZER};
pthread_cond_t  dci_plot_cond[MAX_NOF_RF_DEV]  = {PTHREAD_COND_INITIALIZER,
...
void filter_dci_from_tree(ngscope_tree_t* tree, ngscope_ue_tracker_t* ue_tracker, ngscope_dci_per_sub_t* dci_per_sub)
{
...
void update_ue_dci_per_loc(ngscope_tree_t* tree, ngscope_ue_tracker_t* ue_tracker, int loc_idx, uint32_t tti)
{
                       ...
                       ngscope_dci_per_sub_t* dci_per_sub)
{
  ...
  if (decode_pdsch) {
    ...
    if (decode_single_ue) {
      n = srsran_ngscope_decode_dci_singleUE_yx(&dci_decoder->ue_dl,
                                                &dci_decoder->dl_sf,
                                                &dci_decoder->ue_dl_cfg,
                                                &dci_decoder->pdsch_cfg,
                                                dci_per_sub,
                                                targetRNTI);
    } else {
      ngscope_tree_t tree;
      n = srsran_ngscope_search_all_space_array_yx(&dci_decoder->ue_dl,
                                                   &dci_decoder->dl_sf,
                                                   &dci_decoder->ue_dl_cfg,
                                                   &dci_decoder->pdsch_cfg,
                                                   dci_per_sub,
  ...
  if (decode_SIB) {
    ...
    int  n                          = srsran_ngscope_decode_SIB_yx(
        &dci_decoder->ue_dl, &dci_decoder->dl_sf, &dci_decoder->ue_dl_cfg, &dci_decoder->pdsch_cfg, acks, data);
    if (n > 0) {
                              ...
                              srsran_phich_res_t*    phich_res)
{
  ...
  if (targetRNTI > 0) {
    ...
    ack_available =
        decode_phich(&dci_decoder->ue_dl, &dci_decoder->dl_sf, &dci_decoder->ue_dl_cfg, &ack_list, phich_res);
    if (ack_available) {
...
//                           sf_buffer[decoder_idx].IQ_buffer, rx_softbuffers, decoder_idx);
  ...
  while (!go_exit) {
    ...
    // t1 = timestamp_us();
    pthread_cond_wait(&sf_buffer[rf_idx][decoder_idx].sf_cond, &sf_buffer[rf_idx][decoder_idx].sf_mutex);
